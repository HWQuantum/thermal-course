...
2. Set the total number of time steps $T$ (e.g. 10,000).
3. Set the number of bins $B$ for entropy calculation (e.g. 2, 4, or 8).
4. Initialise a vector of length $T$ to store entropy values $S$.
...
2. Determine neighbour index $j$:
   - If $i = 1$, set $j = 2$.
   - Else if $i = N$, set $j = N - 1$.
   - Otherwise, randomly choose $j = i - 1$ or $j = i + 1$.

3. Swap the values to simulate convection:
   $$
   v[i] \leftrightarrow v[j]
   $$
...
6. For each bin $b$:
   $$
   p_b = \frac{\text{sum of values in bin } b}{\text{sum of all values}}
   $$
...
8. Store $S_t$ in the entropy array.
